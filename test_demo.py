...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    for i, (img_lr, img_hr) in enumerate(data_path):
        ...
        if not is_test:
            ...
            logger.info("{:s} - Saved.".format(img_name + ext))
    ...
    logger.info(f"------> Average runtime of ({mode}) is : {results[f'{mode}_ave_runtime']:.6f} milliseconds")
    if not is_test:
        logger.info(f"------> Average PSNR of ({mode}) is : {results[f'{mode}_ave_psnr']:.6f} dB")
...
def main(args):
    ...
    for k, v in results.items():
        if 'valid_ave_psnr' not in v or 'valid_ave_runtime' not in v:
            logger.warning(f"Skip {k} because valid results are incomplete: {list(v.keys())}")
            continue

        val_psnr = f"{v['valid_ave_psnr']:2.2f}"
        val_time = f"{v['valid_ave_runtime']:3.2f}"
        mem = f"{v.get('valid_memory', 0):2.2f}"

        num_param = f"{v.get('num_parameters', 0):2.3f}"
        flops = f"{v.get('flops', 0):2.2f}"
        acts = f"{v.get('activations', 0):2.2f}"
        conv = f"{int(v.get('num_conv', 0)):4d}"

        if args.include_test and 'test_ave_runtime' in v:
            test_psnr = f"{v.get('test_ave_psnr', 0):2.2f}" if 'test_ave_psnr' in v else "N/A"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_only", action="store_true", help="Only run test (no GT), save SR images")
